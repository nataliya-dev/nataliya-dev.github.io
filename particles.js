...
var pJS = function (tag_id, params) {
  ...
  var repulseElems = document.querySelectorAll(".repulse");
  var repulsePadding = 15;
  ...
  pJS.fn.interact.linkParticles = function (p1, p2) {
    ...
    if (dist <= pJS.particles.line_linked.distance) {
      ...
      for (var i = 1; i < elems_on_line; i++) {
        ...
        y_on_line.push(m * (p1.x + dist_inc) + b);
      }
      ...
      try {
        repulseElems.forEach(function (elem) {
          if (do_link) {
            for (var j = 0; j < x_on_line.length; j++) {
              var pX = x_on_line[j];
              var pY = y_on_line[j];
              if (pX > elem.offsetLeft - repulsePadding && pX < elem.offsetLeft + elem.offsetWidth + repulsePadding && pY > elem.offsetTop - repulsePadding && pY < elem.offsetTop + elem.offsetHeight + repulsePadding) {
                do_link = false;
                break;
              }
            }
